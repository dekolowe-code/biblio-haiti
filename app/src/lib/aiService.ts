...
// Clé API récupérée depuis le fichier .env (VITE_MISTRAL_API_KEY)
...
export async function sendChatMessage(messages: Message[]): Promise<string> {
  if (!API_KEY) {
    console.warn('No VITE_MISTRAL_API_KEY found. Using mock response.')
    // Simuler un délai réseau
    await new Promise(resolve => setTimeout(resolve, 1500))
    return "Ceci est une réponse simulée. Pour activer la véritable IA, veuillez ajouter VITE_MISTRAL_API_KEY dans votre fichier .env."
  }
